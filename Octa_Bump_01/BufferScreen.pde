...
void computerBufferScreen()
{
  ...
  {
    p.updates();
    bufferScreen.pointLight(p.r, p.g, p.b, p.x, p.y, p.z);
    if (debug)
    {
      showLigth(p);
    }
...
void defineLight(int nb)
{
  ...
  for (int i=0; i<nb; i++)
  {
    ...
    lightPos.add(new PtLight(false));
  }
}

void showLigth(PtLight p)
{
  bufferScreen.pushStyle();
  bufferScreen.strokeWeight(10);
  bufferScreen.stroke(p.r, p.g, p.b);
  bufferScreen.point(p.x, p.y, p.z);
