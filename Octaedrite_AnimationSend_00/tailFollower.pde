class Follower
{
  ...
  void displayTail()
  {
    if (tail.size() > 0)
    {
      ...
      for (int i=0; i< tail.size(); i++)
      {
        ...
        float ns = noise(frameCount * 0.25, i * 0.2) * (scaleTail*2);
  ...
  void displayFollower()
  {
    ...
    fill(0, 00, 100);
    rectMode(CENTER);
    rect(0, 0, scaleTail * 2, scaleTail * 2);
  ...
  PVector getLocation()
  {
    return tail.get(tail.size()-1).copy();
  }
}